import { Settings, updateClock} from "./scripts.js";
import { objClock } from "./objClock.js";

let clock = new objClock();
let settings = new Settings();
console.log(settings.theme.customization)

window.wallpaperPropertyListener = {
    applyUserProperties: function (properties) {
        if (properties.ui_font_size) settings.font.size = properties.ui_font_size.value;

        if (properties.ui_use_12h_format) settings.format.use_12h_format = properties.ui_use_12h_format.value;

        if (properties.ui_show_leading_zero) settings.format.show_leading_zero = properties.ui_show_leading_zero.value;

        if (properties.ui_use_const) settings.format.use_const_declaration = properties.ui_use_const.value;
        ...
        if (properties.ui_customize_theme_enable) {
            ...
            settings.theme.customization.enable = true
